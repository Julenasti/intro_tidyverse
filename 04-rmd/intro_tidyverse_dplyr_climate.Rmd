...
load(here("00-raw", "tmax-ona.RData"))
load(here("00-raw", "tmin-ona.RData"))

## CO2 emissions
# here we will download the data from the webpage and read them using the tidyverse
...
5.  Create a new variable (column) with mutate to get average temperature data

```{r mutate, include = FALSE}

# delete
daily <- df_tmax_sel %>%
  full_join(df_tmin_sel) %>% 
  mutate(
    across(c(Tmax, Tmin), function(x){x / 100})
  )
...
2.  Visualise the relationship betweeen average temperature and years
3.  Visualise the relationship betweeen CO^2^ value and average temperature
...
final_plot <- (gg_ye_co_th + gg_ye_co_sec_th) /
  ...
  plot_annotation(
    title = expression(paste("Annual mean temperature and "~CO[2], " missions relationship over the years in Oñati, Gipuzkoa (1950-2017)")),
      # "Annual mean temperature and CO2 emissions relationship over the years in Oñati, Gipuzkoa (1950-2017)",
    caption = "   Data source:
...
ggsave(
  plot = final_plot,
  here("03-figures", "onati_co2_temp.png"),
  width = 20, height = 14
)
